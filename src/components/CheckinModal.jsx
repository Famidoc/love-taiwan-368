...
export default function CheckinModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto">
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl border border-slate-100 overflow-hidden my-8 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="bg-gradient-to-r from-emerald-800 via-emerald-700 to-teal-800 text-white p-5 relative">
          ...
          <div className="flex items-baseline gap-2 mt-1">
            ...
            <h2 className="text-2xl font-black font-serif-tw tracking-wide text-amber-300">
              {district.township}
            </h2>
        ...
        <div className="p-5 max-h-[75vh] overflow-y-auto space-y-5">
              ...
              {district.attractions.map((att, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full p-3 rounded-2xl text-left text-sm flex items-center justify-between transition-all border ${
                      isChecked
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2.5">
              {photos.map((p, index) => (
                <div key={p.id} className="relative aspect-square rounded-2xl overflow-hidden border border-slate-200 group">
                  <img
                    src={p.dataUrl}
                    alt={`照片 ${index + 1}`}
              ...
              {photos.length < 3 && (
                <label className={`aspect-square rounded-2xl border-2 border-dashed border-slate-300 hover:border-emerald-500 bg-slate-50 hover:bg-emerald-50/50 flex flex-col items-center justify-center cursor-pointer transition-all ${
                  ...
                  {isCompressing ? (
                    ...
                    <>
                      <Upload className="w-5 h-5 text-slate-400 group-hover:text-emerald-600 mb-1" />
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full p-3 text-sm bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 focus:outline-none placeholder:text-slate-400"
            />
          </div>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-2.5">
          ...
          <div className="flex items-center justify-end gap-2 w-full sm:w-auto">
            ...
            <button
              type="button"
              onClick={handleSave}
              className="flex-1 sm:flex-initial px-6 py-2.5 text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-1.5"
            >
